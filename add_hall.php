...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt) {
        // Set department_id to NULL if not selected or if belong_to is not 'Department'
        if ($_POST['belong_to'] !== 'Department') {
            $department_id = null;
        }
        
        $stmt->bind_param("issssssssssssii", $hall_type_id, $hall_name, $capacity, $features, $floor_name, $zone, $image_path, $room_availability, $belong_to, $incharge_name, $designation, $incharge_email, $incharge_phone, $school_id, $department_id);
        if ($stmt->execute()) {
